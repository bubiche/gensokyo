---
name: gensokyo
description: Use when the user talks about other Claude Code sessions on this machine or wants to start a new agent/session in some directory, list who is running or waiting, close or stop a session, or bring a departed session back. Also triggers on the cockpit's own words - resident, summon, who, banish, recall, shrine. Runs the gensokyo CLI; never drives tmux directly.
---
...
| "show me Marisa", "switch to Sakuya", "focus 3" | not the CLI: say that the user brings a resident to the front by clicking its tab, or its line in the shrine tab |
